 ...
 *
 * 04.06.2019
 * Переименовал файл двоичных данных.
 * Теперь Camac_data_000.bin
 *
 * 23.05.2019
 *
 * - Переименовать виджет индикации связи "camac_tab" в "gauge_Tansfer", т.к. путаю его с вкладкой основного окна "ui->tab_camac".
 *
 * 20.05.2019
 * Продолжаем "доводить до ума"
 * - Убираем вкладки  "tab_ADP16_Alpha_Spectral", "tab_FISSION"
 * Right click ->page 4 of 4-> Delete
 *  Чтобы в продакшн версии не было лишнего, тем более это еще не работает.
 *
 * - Убрал строку NAF_Data_Label
   - убрал строку label_4
   - Убрал groupBox (Отображение кода команды КАМАК)
   - Убрал строку label_23
   - Убрал QSpinBox
   - Убрал строку label_21

   - Заменил способ смены цвета на кнопках установки бита данных.
     Было  setStyleSheet("background-color: lightgray");
     Стало setStyleSheet(conn_marker_color_green);

   - Рамка Frequency_frame, изменил свойство "frameShape". Было StyledPanel стало Box
 *
 * В этот раз попробую оставить все в рамках Qt-проекта "KK_Tab_15_start_acquisition", без переноса в отдельный, новый проект.
 *
 *
 *
 *
 * 16.05.2019
 * - Все виджеты инидикации перенес на схему отрисовки через "->setStyleSheet"
 * - Добавил выход из ф-ций, если нет подключенных FTDI-устройств
 * - Доработал ф-цию ftdi_answer_parser(int value), - сделал там установку флага  FTDI_Connected.
 * - Сохранил в коммит f0d3014,  https://bitbucket.org/uts-jinr/qt_camac/branch/KK_Tab_15_start_acquisition
 *
 *
 *
 * 15.05.2019
 * Создаю виджеты индикации в дизайнере форм.
 * При таком способе цвет фона не переписывает виджеты.
 * Однако код, ранее менявший цвет виджета :
 *
    ...
    Значит везде придется ставить код :

    ui->$$WIDGET_NAME$$->setStyleSheet("background-color: rgb(255, 0, 0);");
 *
 *
 *
 * 15.05.2019
